...
pub(super) fn migrate_data(source_dir: &Path, dest_dir: &Path, verbose: bool, result: &mut MigrationResult) -> Result<(), String> {
    if verbose {
        log::info!("Migrating data files from Fresh to Rustyll format");
    }
    ...
    // Fresh sites often have JSON data in the routes directory or in specific data folders
    let potential_data_dirs = vec![
        source_dir.join("data"),
        source_dir.join("src/data"),
        source_dir.join("routes/_data"),
        source_dir.join("static/data"),
    ];
    ...
    for data_dir in &potential_data_dirs {
        if data_dir.exists() && data_dir.is_dir() {
            ...
            // Walk the data directory for JSON files
            for entry in WalkDir::new(data_dir)
                ...
                if entry.file_type().is_file() {
                    ...
                    // Filter for JSON and other data files
                    if let Some(extension) = file_path.extension() {
                        ...
                        if ext == "json" || ext == "yaml" || ext == "yml" || ext == "csv" {
                            ...
                            result.changes.push(MigrationChange {
                                ...
                                description: format!("Data file copied from Fresh site").to_string(),
                            });
    ...
    if !found_data {
        ...
        let site_data = r#"{
  "title": "Fresh Site",
  "description": "A site migrated from Fresh to Rustyll",
  "navigation": [
    {
      "text": "Home",
      "url": "/"
    ...
    },
    {
      ...
      "url": "/blog"
        ...
        fs::write(dest_data_dir.join("site.json"), site_data)
            .map_err(|e| format!("Failed to write sample data file: {}", e))?;
        
        result.changes.push(MigrationChange {
            file_path: "_data/site.json".to_string(),
...
This directory contains data files migrated from your Fresh site. These files can be accessed in templates using the `site.data` namespace.
...
The data files in this directory are in JSON, YAML, or CSV format. If you need to add more data:

1. Add new JSON or YAML files to this directory
2. Access them using `site.data.filename`
3. Nested directories can be accessed using dot notation

For example, a file at `_data/authors/info.json` would be accessible as `site.data.authors.info`.
"#;
