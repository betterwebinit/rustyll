...
impl EleventyMigrator {
    pub fn new() -> Self {
        EleventyMigrator
    }
}

impl EngineMigrator for EleventyMigrator {
    fn name(&self) -> &'static str {
        "Eleventy"
    }
    
    fn description(&self) -> &'static str {
        "Migrates Eleventy (11ty) sites to Rustyll format"
    }
    
    fn detect(&self, source_dir: &Path) -> bool {
        // Check for Eleventy-specific files
        source_dir.join(".eleventy.js").exists() || 
        source_dir.join("eleventy.config.js").exists() ||
        source_dir.join("_includes").exists() && source_dir.join("_data").exists()
    }
